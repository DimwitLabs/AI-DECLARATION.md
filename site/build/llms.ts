...
export function renderLlmsTxt(rows: LlmsRow[]): string {
  ...
  const sections: string[] = [
    '# AI-DECLARATION.md',
    '',
    '> The most popular machine-readable open standard for declaring how much AI was involved in building a software project. A project adds an `AI-DECLARATION.md` file to its repository stating an overall involvement level, and optionally a breakdown by process and by component.',
    '',
    ...
    '',
    'The declaration is machine-readable by design: plain Markdown with YAML front matter, followed by a required `## Notes` section for human context. The same file is readable by a person and parseable by a tool, with no separate manifest to keep in sync. At minimum it needs `version`, `level`, and those notes. `processes` and `components` are optional, and the global `level` must be the highest level present among them. Any process not listed is implicitly `none`.',
    '',
    ...
    '',
    '`components` maps file paths or directories to a level, using the same six values. Every field is drawn from a closed vocabulary, which is what makes a declaration machine-readable rather than prose to be interpreted.',
    '',
    ...
    `- [API](${BASE_URL}/api/): read-only JSON API to detect and validate declarations as machine-readable output, no key required`,
